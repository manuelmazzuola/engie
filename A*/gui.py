...
class Interface(goocanvas.Canvas):
	def __init__(self):
		...
		self._lines = []
		self.show()
		print "Click to add new points, then click the middle button"
	...
	def button_press(self, widget, event):
		...
		if self._state == 0:
			if event.button == 1:
				...
				self._state = 1
				print "Click on nodes to link them together, then click the right button"
		#collegamento nodi
		elif self._state == 1:
			if event.button == 3:
				self._state = 2
				self._first = None
				print "Click on the starting node"
	...
	def circle_press(self, widget, target, event):
		if self._state == 1:
			...
			if self._first is None:
				...
				print "Click on the arrival node"
				print self._first
			else:
